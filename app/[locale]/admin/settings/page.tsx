...
import { Save, RefreshCw, Plus, Trash2, ShieldCheck, Cpu, Cloud, Zap } from 'lucide-react';
...
export default function AdminSettingsPage() {
    ...
    const [newCloudflareFallback, setNewCloudflareFallback] = useState('');
    ...
    const fetchSettings = async () => {
        try {
            ...
            setSettings({
                ...
                cloudflareAi: data.cloudflareAi || {
                    apiKey: '',
                    primaryModel: '',
                    fallbackModels: [],
                    isEnabled: true
                },
                otherConfigs: data.otherConfigs || {}
            });
        } catch (error) {
    ...
    const handleSaveCloudflare = async () => {
        ...
        try {
            await api.put('/admin/settings/cloudflare-ai', settings.cloudflareAi);
            toast.success("Réglages Cloudflare mis à jour avec succès");
        } catch (error) {
            toast.error("Erreur lors de la sauvegarde Cloudflare");
        } finally {
    ...
    const addCloudflareFallback = () => {
        if (!newCloudflareFallback || !settings) return;
        setSettings({
            ...settings,
            cloudflareAi: {
                ...settings.cloudflareAi,
                fallbackModels: [...settings.cloudflareAi.fallbackModels, newCloudflareFallback]
            }
        });
        setNewCloudflareFallback('');
    };

    const removeCloudflareFallback = (index: number) => {
        if (!settings) return;
        const newFallbacks = [...settings.cloudflareAi.fallbackModels];
        newFallbacks.splice(index, 1);
        setSettings({
            ...settings,
            cloudflareAi: {
        ...
        <div className="max-w-4xl mx-auto space-y-8">
            ...
            <div className="tech-border bg-black/40 overflow-hidden">
                <div className="p-6 border-b border-primary/20 bg-primary/5 flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-lg font-bold font-mono">CLOUDFLARE_AI_OCR</h2>
                        <p className="text-xs text-foreground/40 font-mono">CONFIGURE_EDGE_AI_ENGINE</p>
                    </div>
                </div>
                ...
                <div className="p-8 space-y-8">
                    <div className="grid md:grid-cols-2 gap-8">
                        <div className="space-y-2">
                            <label className="text-xs font-bold font-mono text-primary/80">CLOUDFLARE_API_KEY</label>
                            <div className="relative group">
                                <input
                                    type="password"
                                    value={settings?.cloudflareAi.apiKey}
                                    onChange={(e) => setSettings(s => s ? ({ ...s, cloudflareAi: { ...s.cloudflareAi, apiKey: e.target.value } }) : null)}
                                    placeholder="CLOUDFLARE_API_TOKEN"
                                    className="w-full px-4 py-2.5 bg-black/40 tech-border focus:ring-2 focus:ring-primary/20 outline-none transition-all font-mono text-sm text-foreground/80"
                                />
                                <Zap className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-primary opacity-0 group-hover:opacity-100 transition-opacity" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                type="text"
                                value={settings?.cloudflareAi.primaryModel}
                                onChange={(e) => setSettings(s => s ? ({ ...s, cloudflareAi: { ...s.cloudflareAi, primaryModel: e.target.value } }) : null)}
                                placeholder="@cf/meta/llama-2-7b-chat-int8"
                                className="w-full px-4 py-2.5 bg-black/40 tech-border focus:ring-2 focus:ring-primary/20 outline-none transition-all font-mono text-sm text-foreground/80"
                            />
                        </div>
                    </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-primary/20">
                        <div className="flex justify-between items-end">
                            ...
                            <div className="flex gap-2 w-full max-w-sm">
                                <input
                                    type="text"
                                    value={newCloudflareFallback}
                                    onChange={(e) => setNewCloudflareFallback(e.target.value)}
                                    placeholder="@cf/google/gemma-7b-it"
                                    className="flex-1 px-3 py-2 bg-black/40 tech-border text-sm font-mono outline-none text-foreground/80"
                                />
                                <button
                                    onClick={addCloudflareFallback}
                                    className="tech-border bg-primary/10 text-primary p-2 hover:bg-primary/20 transition-colors"
                                >
                                    <Plus className="w-4 h-4" />
                                </button>
                        ...
                        <div className="grid gap-2">
                            {settings?.cloudflareAi.fallbackModels.map((model, idx) => (
                                <div key={idx} className="flex items-center justify-between p-3 bg-primary/5 tech-border border-primary/20 group">
                                    <div className="flex items-center gap-3">
                                        <div className="w-6 h-6 tech-border bg-primary/20 flex items-center justify-center text-[10px] font-bold text-primary">
                                            {idx + 1}
                                    ...
                                    <button
                                        onClick={() => removeCloudflareFallback(idx)}
                                        className="p-1.5 text-primary opacity-0 group-hover:opacity-100 hover:bg-primary/10 tech-border transition-all"
                                    >
                                        <Trash2 className="w-4 h-4" />
                                    </button>
                            ...
                            {settings?.cloudflareAi.fallbackModels.length === 0 && (
                                <div className="text-center py-6 tech-border border-dashed border-primary/20 text-foreground/30 text-sm font-mono">
                                    NO_FALLBACK_CONFIGURED
                                </div>
                            )}
                    ...
                    <div className="flex items-center gap-3 pt-6 border-t border-primary/20">
                        <div className="flex-1">
                            <h3 className="text-xs font-bold font-mono text-primary/80">ENABLE_CLOUDFLARE_AI</h3>
                            <p className="text-xs text-foreground/40 font-mono">ENABLE_FOR_FREE_STARTER_PLANS</p>
                        </div>
                        <button
                            onClick={() => setSettings(s => s ? ({ ...s, cloudflareAi: { ...s.cloudflareAi, isEnabled: !s.cloudflareAi.isEnabled } }) : null)}
                            className={cn(
                                "tech-border w-12 h-6 transition-all relative",
                                settings?.cloudflareAi.isEnabled ? "bg-primary/20 border-primary" : "bg-black/40 border-primary/20"
                            )}
                        >
                            <div className={cn(
                                "absolute top-1 w-4 h-4 bg-primary transition-all",
                                settings?.cloudflareAi.isEnabled ? "left-7" : "left-1"
                            )} />
                        </button>
                    </div>
                    ...
                    <div className="pt-6 border-t border-primary/20 flex justify-end">
                        <button
                            ...
                            className="tech-border bg-black/40 flex items-center gap-2 px-6 py-2 text-primary font-mono text-sm hover:bg-primary/10 hover:border-primary/40 transition-all disabled:opacity-50"
                        >
                            {isSavingCloudflare ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
